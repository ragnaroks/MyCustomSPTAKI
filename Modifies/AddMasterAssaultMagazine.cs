...
public class AddMasterAssaultMagazine : IOnLoad {
    private ISptLogger<AddMasterAssaultMagazine> Logger { get; }
    ...
    private MongoId BaseId { get; } = new("6a08f67d799458e8c3913100");
    private MongoId NewId { get; } = new("6a08f67d799458e8c3913101");
    private MongoId RotateId { get; set; } = new("6a08f67d799458e8c3913120");

#pragma warning disable IDE0290 // 使用主构造函数
    public AddMasterAssaultMagazine (ISptLogger<AddMasterAssaultMagazine> logger, DatabaseService databaseService, CustomItemService customItemService, ConfigServer configServer) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = "master assault magazine",ShortName = "Master",Description = "skydust™ master assault magazine"}},
                {"ch",new(){Name = "大师突击步枪弹匣",ShortName = "大师",Description = "skydust™ 大师突击步枪弹匣"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddMasterAssaultMagazine.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddMasterAssaultMagazine.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddMasterAssaultMagazine.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
