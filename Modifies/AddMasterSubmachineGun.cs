...
public class AddMasterSubmachineGun : IOnLoad {
    private ISptLogger<AddMasterSubmachineGun> Logger { get; }
    private DatabaseService DatabaseService { get; }
    private CustomItemService CustomItemService { get; }
    private ConfigServer ConfigServer { get; }
    private Double HandbookPrice { get; } = 150_0000D;
    private MongoId BaseId { get; } = new("6a08f9ed1b820ddcca1db400");
    private MongoId NewId { get; } = new("6a08f9ed1b820ddcca1db401");
    private MongoId RotateId { get; set; } = new("6a08f9ed1b820ddcca1db420");
    private MongoId OriginTemplateId { get; } = ItemTpl.SMG_TDI_KRISS_VECTOR_GEN2_45_ACP_SUBMACHINE_GUN;

#pragma warning disable IDE0290 // 使用主构造函数
    public AddMasterSubmachineGun (ISptLogger<AddMasterSubmachineGun> logger, DatabaseService databaseService, CustomItemService customItemService, ConfigServer configServer) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        if (originItem is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddMasterSubmachineGun.OnLoad() / failed / TemplateId [", this.OriginTemplateId, "] not found"),
                LogTextColor.Yellow
            );
        ...
        if (originItem.Properties is null || originItem.Properties.Slots is null || originItem.Properties.Chambers is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddMasterSubmachineGun.OnLoad() / failed / TemplateId [", this.OriginTemplateId, "] properties error"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        foreach (Slot slot in slots) {
            ...
            slot.Properties.Filters = [
                new SlotFilter(){
                    Filter = [new("6a08f2d0d1e5c3ff0f95b701")]
                }
            ];
            break;
        }
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = "master submachine gun",ShortName = "Master",Description = "skydust™ master submachine gun"}},
                {"ch",new(){Name = "大师冲锋枪",ShortName = "大师",Description = "skydust™ 大师冲锋枪"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddMasterSubmachineGun.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddMasterSubmachineGun.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddMasterSubmachineGun.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
