...
public class AddThermalVisionGoggles : IOnLoad {
    ...
    private MongoId BaseId { get; } = new("67f65b10352e88d7cb840200");
    private MongoId NewId { get; } = new("67f65b10352e88d7cb840201");
    private MongoId RotateId { get; set; } = new("67f65b10352e88d7cb840220");

#pragma warning disable IDE0290 // 使用主构造函数
    public AddThermalVisionGoggles (ISptLogger<AddThermalVisionGoggles> logger, DatabaseService databaseService, CustomItemService customItemService,ItemHelper itemHelper) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = "thermal vision goggles",ShortName = "ThermalVision",Description = "skydust™ thermal vision goggles"}},
                {"ch",new(){Name = "热成像仪",ShortName = "热成像仪",Description = "skydust™ 热成像仪"}}
            },
            OverrideProperties = new() {
                ...
                RoughnessCoef = 0D,
                SpecularCoef = 1.5D,
                MainTexColorCoef = 0.6D,
                MinimumTemperatureValue = 0.2D,
                RampShift = 0D,
                HeatMin = 0D,
                ColdMax = 0D
            }
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddThermalVisionGoggles.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddThermalVisionGoggles.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddThermalVisionGoggles.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
