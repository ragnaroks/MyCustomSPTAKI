...
export default function addNewItemSpecialPistolMagazine1(logger: ILogger,customItemService: CustomItemService,tables: IDatabaseTables) {
  const newId: string = '67dd012b1b3dd0c240cb5350';
  const assortId: string = '67dd012b1b3dd0c240cb5360';

  const templateItem = tables.templates.items[ItemTpl.MAGAZINE_9X19_GLOCK_9X19_17RND] || null;
  if(!templateItem) {
    logger.error('[MyCustomSPTAKI]: 未加入 SpecialPistolMagazine1，错误：模板物品 MAGAZINE_9X19_GLOCK_9X19_17RND 不存在');
    return;
  }

  const newItem: NewItemFromCloneDetails = {
    itemTplToClone: ItemTpl.MAGAZINE_9X19_GLOCK_9X19_17RND,
    newId: newId,
  ...
  if(!createResult.success) {
    logger.error('[MyCustomSPTAKI]: 未加入 SpecialPistolMagazine1，错误：' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[MyCustomSPTAKI]: 已加入 SpecialPistolMagazine1，ID：' + createResult.itemId);
}
