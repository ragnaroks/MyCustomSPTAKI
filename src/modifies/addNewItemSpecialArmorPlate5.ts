...
const baseId: string = '68e277c17e17026fcff79c00';
const assortId: string = idcalc(baseId,0xff);

export default function addNewItemSpecialArmorPlate5(logger: ILogger,customItemService: CustomItemService,tables: IDatabaseTables) {
  const templateItem = tables.templates.items[ItemTpl.ARMORPLATE_GAC_3S15M_BALLISTIC_PLATE] || null;
  if(!templateItem) {
    logger.error('[MyCustomSPTAKI]: 未加入 SpecialArmorPlate5，错误：模板物品 ARMORPLATE_GAC_3S15M_BALLISTIC_PLATE 不存在');
    return;
  }

  const newItem: NewItemFromCloneDetails = {
    itemTplToClone: ItemTpl.ARMORPLATE_GAC_3S15M_BALLISTIC_PLATE,
    newId: baseId,
    ...
    locales: {
      en: {
        name: 'special class-5 armor plate',
        shortName: 'SAP5',
        description: 'special class-5 armor plate'
      },
      ch: {
        name: '特制 5 级插板',
        shortName: 'SAP5',
        description: '特制 5 级插板'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[MyCustomSPTAKI]: 未加入 SpecialArmorPlate5，错误：' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[MyCustomSPTAKI]: 已加入 SpecialArmorPlate5，ID：' + createResult.itemId);
}
