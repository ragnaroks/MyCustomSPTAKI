...
export default function addNewItemSpecialPistolMagazine3(logger:ILogger,customItemService:CustomItemService,tables: IDatabaseTables) {
  ...
  const newItem: NewItemFromCloneDetails = {
    itemTplToClone: ItemTpl.MAGAZINE_9X19_G_SGMT_50RND,
    newId: newId,
    ...
    locales: {
      en: {
        name: 'special pistol magazine type 3',
        shortName: 'SPM3',
  ...
  if(createResult.success) {
    ...
    logger.success('[MyCustomSPTAKI]: 已加入 SpecialPistolMagazine3，ID：' + createResult.itemId);
  } else {
    logger.error('[MyCustomSPTAKI]: 未加入 SpecialPistolMagazine3，错误：' + createResult.errors.join('、'));
  }
}
